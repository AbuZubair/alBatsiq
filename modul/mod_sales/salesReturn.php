<?php
	...
	$cek = mysql_query("SELECT * FROM sales where SALES_CODE = '002' AND SALES_DATE LIKE '$tahun-$bulan%'");
				...
				<div class='content-left'>
					<table width='100%'>
						<tr>
							...
							<td><input type='text' name='slretnumber' id='slretnumber' value='SALERET/$tahun/$bulan/$num' disabled></td>
						</tr>
						<tr>
				...
				<div style='clear:both;'>	
					<table width='100%'>
						...
						<tr>
							...
							<td>
								<a class='button3' onclick='return addRecordSale()' id='buttonInsert'><i class='fa fa-plus-circle'></i> Insert</a>
							</td>
					...
					<form method=POST action='modul/mod_sales/aksi_sale.php?act=addSalesRet' id='form1' onsubmit='return setValues()'>
						<table width='100%' id='record'>
							<input type='hidden' name='slretnumber' id='slretnumber' value='SALERET/$tahun/$bulan/$num'>
...
<script>
 ...
 function outstandingReturn(){
	...
	//		document.getElementById("alertlocunit").innerHTML = "";
			...
			window.open("modul/mod_sales/selectSales.php?id=default&page=1", "popuppage", "width=500,height=600,toolbar=no,scrollbars=yes,location=no,statusbar=no,menubar=no,resizable=yes,fullscreen=no");
	//	}
	}
    ...
    // this gets called from the popup window and updates the field with a new value		
		...
		if(rowCount == 0){
				...
				var vals = nilai;
				 ...
				 var url = 'modul/mod_sales/get_sale.php?q=' + vals;
				 xhr.open('GET', url, false);
				 xhr.onreadystatechange = function () {
				 if (xhr.readyState===4 && xhr.status===200) {
				 var x =  JSON.parse(xhr.responseText);
				 var count = x.length;
				 document.getElementById("insertlocunit").value = x[0].locid;
				 var ceknum = parseInt(x[0].amt),
					 ...
					 for(var i=0; i<count; i++) {
						...
						var ceknum = parseInt(x[i].diskon),
							numfix = ceknum.toFixed(2),
							num = numfix.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
						val5.value = num;
						var read = document.createAttribute("readonly");
